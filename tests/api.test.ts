...
describe("expect* grammar", () => {
  ...
  it("every error factory carries its code", () => {
    ...
    expect(EnvelopeError.msg("bad").message).toContain("bad");
  });
});
...
describe("salt", () => {
  ...
  it("addSalt options", () => {
    ...
    expect(() => e.addSalt({ salt: new Uint8Array(2) })).toThrow(EnvelopeError);
...
describe("codable", () => {
  it("codec, toCbor, toUR, fromCbor, fromBytes", () => {
    ...
    expect(Envelope.codec.decode(alice.untaggedCbor()).digest().equals(alice.digest())).toBe(true);
...
describe("pipe and format options", () => {
  ...
  it("format, hex, diagnostic, summary options", () => {
    ...
    expect(Envelope.from(IS_A).summary({ context: "none" })).toBe("'isA'");
