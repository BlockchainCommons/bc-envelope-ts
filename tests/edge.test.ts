...
describe("Edge Extension", () => {
  ...
  describe("Edges container operations", () => {
    ...
    it("test_edges_container_iter", () => {
      ...
      const count = Array.from(edges).length;
      expect(count).toBe(2);
    });
  });
  ...
  // ===================================================================
  // Section 8: the Edges container
  // ===================================================================

  describe("Edges container", () => {
  ...
  describe("edgesMatching — filtered retrieval", () => {
    ...
    it("test_edges_matching_by_source", () => {
      ...
      const matching1 = doc.edgesMatching({ source: alice });
      expect(matching1.length).toBe(1);

      const matching2 = doc.edgesMatching({ source: bob });
      expect(matching2.length).toBe(1);

      const carol = xidLike("Carol");
      const matching3 = doc.edgesMatching({ source: carol });
      expect(matching3.length).toBe(0);
    });
    ...
    it("test_edges_matching_by_target", () => {
      ...
      const matching1 = doc.edgesMatching({ target: alice });
      expect(matching1.length).toBe(1);

      const matching2 = doc.edgesMatching({ target: bob });
      expect(matching2.length).toBe(1);
    });
    ...
    it("test_edges_matching_by_subject", () => {
      ...
      const matching1 = doc.edgesMatching({ subject: subjectFilter });
      expect(matching1.length).toBe(1);

      const subjectFilter2 = Envelope.from("nonexistent");
      const matching2 = doc.edgesMatching({ subject: subjectFilter2 });
