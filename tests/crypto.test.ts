...
import { SigningPrivateKey, PrivateKeys, PrivateKeyBase } from "@blockchaincommons/components";
...
// Alice's key exactly as the reference derives it (`alice_private_key().ed25519_signing_private_key()`).
function alicePrivateKey(): SigningPrivateKey {
  return PrivateKeyBase.from(ALICE_SEED).ed25519SigningPrivateKey();
}
...
describe("Crypto Tests", () => {
  ...
  describe("multi-recipient", () => {
    it("should encrypt to multiple recipients", () => {
      const bob = PrivateKeys.generate();
      const carol = PrivateKeys.generate();
      const alice = PrivateKeys.generate();
  ...
  describe("visible signature multi-recipient", () => {
    it("should sign then encrypt to multiple recipients with visible signature", () => {
      ...
      const bob = PrivateKeys.generate();
      const carol = PrivateKeys.generate();
      const eve = PrivateKeys.generate();
