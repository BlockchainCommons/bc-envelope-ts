...
/** Element `i` of a non-empty list, wrapping around. */
...
export function* generated(): Generator<E> {
  ...
  for (const subject of VOCAB_SUBJECTS) {
    for (let n = 0; n <= 3; n++) {
      ...
      for (let a = 0; a < n; a++) {
        const pred = cycle(VOCAB_PREDS, a + i);
        let obj = cycle(VOCAB_OBJS, a * 2 + i);
        if ((a + n + i) % 3 === 0)
          obj = node(obj, [[cycle(VOCAB_PREDS, a + 1), cycle(VOCAB_OBJS, a + 3)]]);
      ...
      if (n >= 1) {
        yield { k: "elide", e, removing: [cycle(assertions, 0)[1]] };
        if (i % 3 === 0)
...
/** A `hasRecipient` envelope the reference produced (`examples/dump_recipient.rs`): "Hello.", key 07…07, Bob from SEED_B. */
export const RUST_RECIPIENT_UR =
  "ur:";
...
const saltedNode: E = {
  k: "nodeEnv",
  subject: text("A"),
  assertions: [salted(assertion(text("p"), text("o")))],
};
const elidedInner: E = { k: "elide", e: saltedNode, removing: [assertion(text("p"), text("o"))] };
...
const signedWithJunk: E = node(text("Hello."), [[kv(3), wrap(text("x"))]]);
/** The wire findings (B1, B2, B3, B4, B6, B11) as recipes. */
export const WIRE: E[] = [
  node(text("Alice"), [
    [text("knows"), text("Bob")],
    [text("knows"), text("Bob")],
  ]),
  {
    k: "nodeEnv",
    subject: text("Alice"),
    assertions: [assertion(text("knows"), text("Bob")), assertion(text("knows"), text("Bob"))],
  },
  saltedNode,
  elidedInner,
  { k: "op", op: "roundtrip", e: elidedInner },
  date(0),
  date(1657512000000),
  ...
  {
    k: "op",
    op: "addAssertionEnvelope",
    e: text("A"),
    assertion: text("not-an-assertion"),
    salt: true,
  },
  { k: "op", op: "addAssertionEnvelope", e: text("A"), assertion: text("not-an-assertion") },
  {
    k: "op",
    op: "addAssertionEnvelope",
    e: text("A"),
    assertion: salted(assertion(text("p"), text("o"))),
  },
  ...
  },
  {
    k: "op",
    op: "encryptSubject",
    e: node(text("Alice"), [[kv(1), text("Bob")]]),
    key: KEY,
    nonce: NONCE_A,
  },
];
/** Rejections, compared by code on both sides (B7, B9, B17). */
export const REJECT: E[] = [
  { k: "op", op: "encryptSubject", e: encrypted, key: KEY7 },
  {
    k: "op",
    op: "encryptSubject",
    e: { k: "elide", e: text("Hello."), removing: [text("Hello.")] },
    key: KEY7,
  },
  { k: "op", op: "decryptSubject", e: text("Hello."), key: KEY7 },
  { k: "op", op: "decryptSubject", e: encrypted, key: KEY2 },
  { k: "op", op: "compress", e: encrypted },
  { k: "op", op: "compress", e: { k: "elide", e: text("Hello."), removing: [text("Hello.")] } },
  { k: "op", op: "decompress", e: text("Hello.") },
  { k: "op", op: "decryptToRecipient", e: recipientEnv, recipientSeed: SEED_A },
  { k: "op", op: "decryptToRecipient", e: encrypted, recipientSeed: SEED_B },
  { k: "op", op: "decryptToRecipient", e: text("Hello."), recipientSeed: SEED_B },
  { k: "op", op: "attachmentPayload", e: text("Hello.") },
  { k: "op", op: "expectAttachment", e: text("Hello.") },
  { k: "op", op: "requestFrom", e: { k: "request", func: "f", id: ARID_1, params: [] }, func: "g" },
  { k: "op", op: "verify", e: signedWithJunk, seed: SEED_A },
  { k: "op", op: "decryptToRecipient", e: recipientEnv, recipientSeed: SEED_B },
];
export const DOMAIN_CASES: string[] = [
  "node.emptyAssertions",
  "knownValue.-1",
  "knownValue.1.5",
  "setPosition.1.5",
  "setPosition.-1",
...
export const categories: Record<string, () => Generator<Recipe>> = {
  ...
  domain: function* () {
    for (const c of DOMAIN_CASES) yield { e: { k: "domain", case: c }, out: ["format"] };
  },
};
