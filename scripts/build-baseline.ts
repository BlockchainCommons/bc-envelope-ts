 ...
 * baseline keeps the frozen behaviour of its dependencies even after
...
for (const dir of readdirSync(parent)) {
  ...
  // its own (much older) frozen baseline.
  if (depName !== pkg.name && depName !== "@blockchaincommons/dcbor") alias[depName] = join(bl, f);
}

// Rebuild only with historical source and matching dependency revisions.
await build({
  config: false,
...
commit \`${commit}\`, the frozen wire-format reference. Sibling
...
frozen behaviour of its dependencies after they change.
