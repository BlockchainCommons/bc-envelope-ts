 ...
 * `@blockchaincommons/components` so envelope consumers can keep using
 * `import { EncryptedMessage } from "@blockchaincommons/envelope"`.
 *
 * The `@blockchaincommons/components` `EncryptedMessage` is the canonical implementation
 ...
 * `export { ... } from "@blockchaincommons/components"`. A separate `export ... from` would
