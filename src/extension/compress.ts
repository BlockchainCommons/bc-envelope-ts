...
/**
 * Re-export the canonical {@link Compressed} from `@blockchaincommons/components` so
 * envelope consumers can keep using `import { Compressed } from
 * "@blockchaincommons/envelope"`. This ensures one source of truth for the compressed-
 * envelope wire format and avoids drift with `bc-components-rust`.
 */
export { Compressed } from "@blockchaincommons/components";
